...
def loop_function(box_vector):
    ...
    move_pose_arm(0.3,0,0.08,0,0,0)  # remove
    move_pose_arm(0.35,0,0.05,0,0,0)  # remove
    #move_pose_arm(box_vector[0]-.15,box_vector[1],box_vector[2]-.14,0,pi/2,0)
    rospy.sleep(1)
    rospy.loginfo("Closing gripper to 0.4")	
    move_joint_hand(0.45)
    rospy.sleep(1)
    rospy.loginfo("Moving arm to target location")	
    #move_pose_arm(box_vector[0]-.15,box_vector[1],box_vector[2]+.1,0,pi/2,0)
    #move_pose_arm(0.4,0,0.24,0,0,-pi/2)
    #rospy.sleep(1)
    move_pose_arm(0,-0.4,0.18,0,0,-pi/2)
    rospy.sleep(1)
    rospy.loginfo("Opening gripper")	
    move_joint_hand(0)
    rospy.sleep(1)
    rospy.loginfo("Moving arm to Home point")	
    move_pose_arm(0,-0.4,0.38,0,0,-pi/2)
    rospy.sleep(1)
    move_pose_arm(0.4,0,0.6,0,0.8,0)
