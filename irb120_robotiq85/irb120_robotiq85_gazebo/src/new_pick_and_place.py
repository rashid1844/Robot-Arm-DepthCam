...
class Pick_Place:
	...
	self.get_box_pose_from_camera()
        print(self.pose_object.position)

        self.pose_place = Pose()
        self.pose_place.position.x = 0.
        self.pose_place.position.y = -0.5  #self.pose_object.position.y -0.1
        self.pose_place.position.z = 0.3    #self.pose_object.position.z - 0.1
        self.pose_place.orientation = Quaternion(*quaternion_from_euler(0.0, 0, 0))
    ...
    def get_box_pose_from_camera(self):
        self.pose_object.position.x= 0.5
        self.pose_object.position.y= 0
        self.pose_object.position.z= 0.2 # TODO: add from subscriber
        #rospy.Subscriber('/block_pose', geometry_msgs.msg.Pose, callback)
        #rospy.spin()  

    def callback(data):
        self.box_vector = [data.position.x, data.position.y, data.position.z]
    ...
    def add_object(self, name):
        ...
        size = (self.object_width,self.object_width,self.object_width)
    ...
    def generate_grasps(self, pose, width):
        ...
        now = rospy.Time.now()
        for angle in numpy.arange(0.0, numpy.deg2rad(360.0), numpy.deg2rad(180.0)):
            ...
            q = quaternion_from_euler(0.0, numpy.deg2rad(0.0), angle)
            ...
            grasp.pre_grasp_approach.direction.vector.x = -0.3
            ...
            grasp.max_contact_force = 1  # TODO: set grip force
            ...
            traj = JointTrajectoryPoint()
            traj.positions.append(0.25)  # TODO: set close 
    ...
    def generate_places(self, target):
        ...
        place.post_place_retreat.direction.vector.y = 0.2
        place.post_place_retreat.direction.vector.z = 0
